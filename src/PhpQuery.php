    ...
    implements \Iterator, \Countable, \ArrayAccess
{
    ...
    public function __construct($document)
    {
        if ( ! is_object($document) )
            $document = new DOMDocumentWrapper($document);

        $this->documentWrapper = $document;
...
//        $version = '1.0';
//
//        $document = new \DOMDocument($version, $charset);
...
//        print("</pre>\n");
    ...
    {
        return extension_loaded('mbstring') && self::$mbstringSupport
            ? mb_eregi('\w', $char)
            : preg_match('@\w@', $char);
    }
...
//				$i = $i+2;
    ...
    protected function runQuery($XQuery, $selector = null, $compare = null)
    {
        ...
        foreach ( $this->stack(array( 1, 9, 13 )) as $k => $stackNode ) {
            ...
            // XXX tmp ?
            $xpath = $this->getNodeXpath($stackNode, 'html');
    ...
    public function find($selectors, $context = null, $noHistory = false)
    {
        ...
        foreach ( $queries as $selector ) {
            ...
            foreach ( $selector as $s ) {
                // TAG
                $isTag = extension_loaded('mbstring') && self::$mbstringSupport
                    ? mb_ereg_match('^[\w|\||-]+$', $s) || $s == '*'
                    : preg_match('@^[\w|\||-]+$@', $s) || $s == '*';
                if ( $isTag ) {
                    $XQuery .= $s;
...
//								$selected = true;
    ...
    public function each($callback, $param1 = null, $param2 = null, $param3 = null)
    {
        ...
        foreach ( $this->elements as $v )
            self::callbackRun($callback, array( $v ), $paramStructure);
        return $this;
    }
...
//        return $this->newInstance(
//            //call_user_func_array(array( '\\PhpQuery\\PhpQuery', 'map' ), $params)
//			self::map2($this->elements, $callback)
//        );
    }
     ...
     */
    public static function map2($array, $callback, $param1 = null, $param2 = null, $param3 = null)
    {
        $result = array();
        $paramStructure = null;
        if ( func_num_args() > 2 ) {
